...
local dir = options.root .. "/command_palette/page"
...
local function palette()
  local config = options.config.theme
...
function M.stop()
  for _, item in pairs({ watch, screens, closing, shrinking, warmed }) do
    if item.stop then
      item:stop()
    end
  end
  ...
  view, watch, screens, queued, closing, shrinking, warmed = nil, nil, nil, nil, nil, nil, nil
  waiting, sent, stack = {}, {}, {}
  loaded, warming = false, false
end
