...
local clipboard = require("command_palette.clipboard")
local emoji = require("command_palette.emoji")
local files = require("command_palette.files")
...
local function preview(row, cb)
  ...
  local task = tasks.new(tool, function(_, out)
    cb(ansi.html(out or ""))
  end, {
    "--color=always",
    "--paging=never",
...
local function action_rows()
  ...
  for _, action in ipairs(actions) do
    if (action.name ~= "screenshot" or screenshots) and (action.name ~= "clipboard" or settings().clipboard) then
      local commands = {}
